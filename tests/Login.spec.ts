...
test('TL-12-6 HOMEWORK #4 Successful authorization and order deletion WITH API', async ({ request }) => {
  const apiClient = await ApiClient.getInstance(request);
  ...
  const responseAfterDelete = await request.get(`https://backend.tallinn-learning.ee/orders/${orderId}`, {
    headers: {
      Authorization: `Bearer ${apiClient.jwt}`,
    },
  });
