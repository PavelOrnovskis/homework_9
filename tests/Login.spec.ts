...
test.describe('Login tests', async () => {
    ...
    test('TL-12-3  HOMEWORK#1 Successful authorization, order creation and order status WITHOUT API', async ({ request }) => {
  ...
  test('TL-12-4 HOMEWORK#2 Successful authorization, order creation and order deletion without API', async ({ request }) => {
    const responseLogin = await request.post(`https://backend.tallinn-learning.ee/login/student`, {
      data: LoginDTO.createLoginWithCorrectData(),
    })
    expect(responseLogin.status()).toBe(StatusCodes.OK);
  ...
  test('TL-12-5 HOMEWORK #3 Successful authorization and order creation with status WITH API', async ({ request }) => {
    const apiClient = await ApiClient.getInstance(request);
    const createOrderWithId = await apiClient.createOrderAndReturnOrderId();
    const responseCheckOrder = await request.get(`https://backend.tallinn-learning.ee/orders/${createOrderWithId}`, {
      headers: {
        Authorization: "Bearer " + apiClient.jwt
      }
    })
...
test('TL-12-6 HOMEWORK #4 Successful authorization and order deletion by status number WITH API', async ({ request }) => {
  const apiClient = await ApiClient.getInstance(request);
  const orderId = await apiClient.createOrderAndReturnOrderId();
  const responseCheckOrder = await request.get(`https://backend.tallinn-learning.ee/orders/${orderId}`, {
    headers: {
      Authorization: "Bearer " + apiClient.jwt
    }
  })
  ...
  const deleteResponse = await request.delete(`https://backend.tallinn-learning.ee/orders/${orderId}`, {
    headers: {
      Authorization: "Bearer " + apiClient.jwt
    }
  });
  ...
  console.log('Awaiting for delete: ', deleteResponseBody);
